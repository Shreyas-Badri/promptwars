...
async def calculate_overlap(node1_id: str, node2_id: str, db: AsyncSession):
    u1 = safe_uuid(node1_id)
    u2 = safe_uuid(node2_id)
    
    # Fallback for demo mock IDs like "2" and "7"
    if not u1 or not u2:
        return {
            "overlap_score": 75,
            "evidence": [
    ...
    stmt1 = select(Relationship.target_node_id).where(Relationship.source_node_id == u1)
    stmt2 = select(Relationship.target_node_id).where(Relationship.source_node_id == u2)
